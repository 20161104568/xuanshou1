...
int load()
{
    FILE *fp1;
    int i,j=0;
    fp1=fopen("//Users//a20161104568//Desktop//2.numbers","r");
    for(i=0; i<M; i++)
        fscanf(fp1,"%s%d%d%d%d%d",s[i].name,&s[i].score[j],&s[i].score[j+1],&s[i].score[j+2],&s[i].score[j+3],&s[i].score[j+4]);
    ...
    FILE *fp2;
    int m,n=0;
    fp2=fopen("//Users//a20161104568//Desktop//2.numbers","r");
    for(m=i; i<M; m++)
        fscanf(fp2,"%s%d%d%d%d%d",s[m].name,&s[m].score[n],&s[m].score[n+1],&s[m].score[n+2],&s[m].score[n+3],&s[m].score[n+4]);
    printf("%d 位裁判的信息读入完毕，随意按一键......\n",M);
    getchar(); getchar();
    fclose(fp2);
    return 0;
}
...
void save()
{
    ...
    fp=fopen("//Users//a20161104568//Desktop//6.numbers","w");
...
void main()
{
    ...
    while(1)
    {
        do
        {
            ...
            printf("2、文件载入选手 裁判信息\n\n");
            printf("3、计算歌手的最终得分并排序\n\n");
            printf("4、保存歌手的得分结果及最终名次\n\n");
            printf("5、退出\n\n");
            printf("请您选择：");
            scanf("%d",&choice);
